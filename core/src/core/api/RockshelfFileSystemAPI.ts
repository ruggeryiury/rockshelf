...
export class RockshelfFileSystemAPI {
  ...
  static getDownloadedDXPatchFile = (commitHash: string): FilePath => DirPath.of(app.getPath('userData')).gotoFile(`DownloadedContent/${commitHash}.zip`)
  static packagesCacheFile = (): FilePath => DirPath.of(app.getPath('userData')).gotoFile('package.cache.json')
  static pythonEnvScriptFile = (): FilePath => DirPath.of(app.getPath('userData')).gotoFile(`Python/${platform() === 'win32' ? 'Scripts' : 'bin'}/python.exe`)
  ...
  static userConfigFile = (): FilePath => DirPath.of(app.getPath('userData')).gotoFile('user_config.json')
  static tempFile = (ext: string = 'bin'): FilePath => FilePath.of(temporaryFile({ extension: ext }))

  // #region Folders

  static appDownloadableContentDir = (): DirPath => DirPath.of(app.getPath('userData')).gotoDir('DownloadedContent')
  static appTempDir = (): DirPath => DirPath.of(app.getPath('userData')).gotoDir('AppTemp')
  static tempDir = (): DirPath => FilePath.of(temporaryFile({ extension: 'bin' })).gotoDir('.')
  static appUserDataDir = (): DirPath => DirPath.of(app.getPath('userData'))
  ...
  static pythonEnvDir = () => DirPath.of(app.getPath('userData')).gotoDir('Python')
  static pythonEnvScriptDir = (): FilePath => DirPath.of(app.getPath('userData')).gotoFile(`Python/${platform() === 'win32' ? 'Scripts' : 'bin'}`)
  ...
  static init = async (app: Electron.App): Promise<void> => {
    ...
    const deprecated: (FilePath | DirPath)[] = [DirPath.of(app.getPath('userData')).gotoDir('DownloadedPackages'), DirPath.of(app.getPath('userData')).gotoDir('AppTemp')]
