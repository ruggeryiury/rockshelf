...
import { type DirPath, type DirPathLikeTypes, pathLikeToDirPath, pathLikeToFilePath, randomBytesFromRanges } from 'node-lib'
import { temporaryDirectory, temporaryFile } from 'tempy'
import { useDefaultOptions } from 'use-default-options'

import { type RB3ExtractionTempFolderObject, RB3File, type RB3FileJSONRepresentation } from '../rb3p/RB3File'
import { BinaryAPI, DTAParser, EDATFile, MOGGFile, type PKGExtractionTempFolderObject, PKGFile, type PKGFileJSONRepresentation, type PKGFileSongPackageStatObject, PythonAPI, type RB3PackageLikeType, type STFSExtractionTempFolderObject, STFSFile, type STFSFileJSONRepresentation, type STFSFileStatObject, type SelectedSongForExtractionObject, type SupportedRB3PackageFileType, TextureFile } from '../rbtools'
import { type DTAFileBatchUpdateObject, type DTAFileUpdateObject, type RB3CompatibleDTAFile, getUnpackedFilesPathFromRootExtraction } from '../rbtools/lib.exports'
 ...
 * @param {DirPathLikeTypes} devhdd0DirPath A valid RPCS3 `dev_hdd0` folder path.
 ...
 * @returns {Promise<RPCS3PackageExtractionObject | false>}
 */
export const extractPackagesForRPCS3 = async (win: BrowserWindow, packages: (RB3File | RB3PackageLikeType)[], devhdd0DirPath: DirPathLikeTypes, packageFolderName: string, options?: RPCS3ExtractionOptions): Promise<RPCS3PackageExtractionObject | false> => {
	const { forceEncryption, songs, updates, updateAllSongs, overwritePackFolder } = useDefaultOptions<RPCS3ExtractionOptions>(
		{
	...
	// sendRendererConsole(win, { packages, devhdd0DirPath, packageFolderName, options })
	...
	const devhdd0 = pathLikeToDirPath(devhdd0DirPath)
	...
	else if (!newFolder.exists) await newFolder.mkDir()
