...
import { type UserConfigObject, addHandler } from '../../core.exports'
...
export class SongDownloadQueueAPI {
	 ...
	 */
	static init(): void {
		addHandler('songDownloadQueue.downloadSong', async (_, __, options: RhythmverseDownloadSongOptions) => {
		...
		addHandler('songDownloadQueue.cleanDownloadQueue', async () => {
			queue = []
		})

		addHandler('songDownloadQueue.cancelDownload', async (_, __, hash: string) => {
