...
import { type FilePathJSONRepresentation, Hex, pathLikeToDirPath } from 'node-lib'
...
export class RockshelfGitHubAPI {
	...
	static async getInstalledDeluxeData(): Promise<DeluxeInstalledData | false> {
		...
		const dxHDR = RockshelfFileSystemAPI.dxHDRFile(devhdd0)
		const dxARK = RockshelfFileSystemAPI.dxARKFile(devhdd0)
		...
		try {
			const commitsAhead = await this.checkCommitsAhead(dxVersion)
			const latestDXVersion = commitsAhead.base_commit.sha.slice(0, 7)

			const commitData = await this.getCommitDataFromCommitHash(dxVersion)
			const downloadedDXPatchFile = RockshelfFileSystemAPI.getDownloadedDXPatchFile(latestDXVersion)
