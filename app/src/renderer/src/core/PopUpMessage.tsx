...
export function PopUpMessage() {
  ...
  return (
    <section id="PopUpMessages" className="absolute! h-full w-full">
      <AnimatedDiv condition={Boolean(msgObject)} {...genAnimation({ opacity: true })} className={clsx('z-50 mt-4 mr-4 ml-auto w-fit max-w-[30%] flex-row! items-start border bg-black/90 p-4 backdrop-blur-md', msgObject?.type === 'warn' ? 'border-yellow-500' : msgObject?.type === 'error' ? 'border-red-500' : '')}>
        {msgObject && (
          <>
            ...
            <p className="text-xs wrap-anywhere">
              <TransComponent i18nKey={`${msgObject.type}${msgObject.method.at(0)?.toUpperCase()}${msgObject.method.slice(1)}${msgObject.code.at(0)?.toUpperCase()}${msgObject.code.slice(1)}`} values={msgObject.messageValues} />
            </p>
          </>
        )}
      </AnimatedDiv>
