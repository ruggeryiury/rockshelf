...
import { UserDataAPI, getLocaleStringFromRenderer, sendMessageBox, useHandler } from '../core.exports'
import { RB3File, type RB3FileJSONRepresentation } from '../lib.exports'
import { PKGFile, type PKGFileJSONRepresentation, STFSFile, type STFSFileJSONRepresentation } from '../lib/rbtools'
...
export const selectorRPCS3Exe = useHandler(async (win, _): Promise<string | false> => {
	const rpcs3ExeFilterName = platform() === 'win32' ? await getLocaleStringFromRenderer(win, 'rpcs3Exe') : await getLocaleStringFromRenderer(win, 'appImageFile')
	const selection = await dialog.showOpenDialog({ properties: ['openFile'], filters: [{ name: rpcs3ExeFilterName, extensions: [platform() === 'win32' ? 'exe' : 'AppImage'] }] })
	...
	try {
		const rpcs3Exe = UserDataAPI.isValidRPCS3ExePath(selection.filePaths[0])
