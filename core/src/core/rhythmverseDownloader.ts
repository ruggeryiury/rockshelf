...
let queue: RhythmverseSongDownload[] = []

export const sendRhythmverseQueue = (win: BrowserWindow, queue: RhythmverseSongDownload[]): true => {
...
export class RhythmverseSongDownload {
  downloadLink: string
  id: string
  hash: string
  albumURL: string
  ...
  static async getDownloadedContentData(): Promise<STFSFileJSONRepresentation[]> {
    ...
    const contentFolder = RockshelfFileSys.appDownloadableContentDir()
...
export const initRhythmverseDownloader = async () => {
  ipcMain.handle('rhythmverseDownloadSong', async (event, options: RhythmverseDownloadSongOptions, userOptions: UserConfigObject) => {
    ...
    const instance = new RhythmverseSongDownload(options, userOptions)
