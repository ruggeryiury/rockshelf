...
import { type DirPath, pathLikeToDirPath, pathLikeToFilePath } from 'node-lib'

import { RockshelfFileSystemAPI, getLocaleStringFromRenderer, sendMessageBox, useHandler } from '../core.exports'
import { userData } from '../init'
import { ImageFile, TextureFile } from '../lib/rbtools'

export const openExternalURL = useHandler(async (_, __, url: string): Promise<void> => await shell.openExternal(url))

export type RockshelfFileSystemAPItemCommand = 'coreModuleRootDir' | 'appUserDataDir' | 'appDownloadableContentDir' | 'rb1UsrDir' | 'rb2UsrDir' | 'rb3UsrDir' | 'appDocumentsDir'
...
export const openRockshelfFSDir = useHandler(async (_, __, command: RockshelfFileSystemAPItemCommand): Promise<boolean> => {
	...
	switch (command) {
		...
		case 'appDocumentsDir': {
			path = RockshelfFileSystemAPI.appDocumentsDir()
			break
		}
		case 'rb1UsrDir':
		case 'rb2UsrDir':
		case 'rb3UsrDir': {
			if (!userData.userConfig || !userData.vfs) throw new Error('User config data is not loaded into memory.')
			const devhdd0 = userData.vfs.devhdd0
