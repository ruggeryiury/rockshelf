...
import { DTAIO, type RB3CompatibleDTAFile, genAudioFileStructure, rankCalculator } from '../lib.exports'
		...
		if (song.multitrack) {
			...
			drumKitWavPath =
				song.tracks_count[0] === 2
					? options.hasSeparated2xWavFiles && song.doubleKick
						? isAbsolute(options.drums2xWavName)
							? FilePath.of(formatStringFromDTA(song, options.drums2xWavName))
							: FilePath.of(songFolderPath.path, formatStringFromDTA(song, options.drums2xWavName))
						: isAbsolute(options.drumsWavName)
							? FilePath.of(formatStringFromDTA(song, options.drumsWavName))
							: FilePath.of(songFolderPath.path, formatStringFromDTA(song, options.drumsWavName))
					: song.tracks_count[0] > 2
						? options.hasSeparated2xWavFiles && song.doubleKick
							? isAbsolute(options.drumKit2xWavName)
								? FilePath.of(formatStringFromDTA(song, options.drumKit2xWavName))
								: FilePath.of(songFolderPath.path, formatStringFromDTA(song, options.drumKit2xWavName))
							: isAbsolute(options.drumKitWavName)
								? FilePath.of(formatStringFromDTA(song, options.drumKitWavName))
								: FilePath.of(songFolderPath.path, formatStringFromDTA(song, options.drumKitWavName))
						: undefined
