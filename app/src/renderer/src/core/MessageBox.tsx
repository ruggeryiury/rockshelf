  ...
  return (
    <section id="msgObject" className="absolute! h-full w-full">
      <AnimatedDiv
        ...
        className={clsx('z-50 mt-4 mr-4 ml-auto w-fit max-w-[30%] flex-row! items-start border bg-black/90 p-2 backdrop-blur-md', msgObject && msgObject.type === 'warn' ? 'border-yellow-500' : msgObject && msgObject.type === 'error' ? 'border-red-500' : msgObject && msgObject.type === 'success' ? 'border-green-500' : '')}
        onClick={() => {
          if (msgObject && timeout !== null) {
          ...
          <>
            {msgObject.type === 'loading' ? (
              ...
              <div className="flex-row! items-start">
                ...
                <div>
                  <h1 className="mb-0.5 border-b border-neutral-800 text-sm!">{t(msgObject.type)}</h1>
