...
import { DirPath, type DirPathLikeTypes, FilePath, type FilePathLikeTypes, pathLikeToDirPath, pathLikeToFilePath } from 'node-lib'
...
export class RockshelfFileSystemAPI {
	...
	static rb3ConfigFile = (emulatorDir: DirPathLikeTypes): FilePath => pathLikeToDirPath(emulatorDir).gotoFile(`${platform() === 'win32' ? 'config/custom_configs' : 'custom_configs'}/config_BLUS30463.yml`)
	static userConfigFile = (): FilePath => documents().gotoFile('rockshelf.config.json')
	...
	static init = async (app: Electron.App): Promise<void> => {
		...
		const deprecated: (FilePath | DirPath)[] = [userData().gotoDir('DownloadedPackages'), userData().gotoDir('AppTemp'), userData().gotoDir('DownloadedContent'), userData().gotoFile('package.cache.json'), userData().gotoFile('user_config.json'), documents().gotoFile('user_config.json')]
