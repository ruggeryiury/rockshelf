...
export const initMainProcessHandlers = (): void => {
  const handlers: InitHandlersArray = [
    ...
    ['openUserDataFolder', async (win) => await UserConfigAPI.openUserDataDir(win)],
    ['playRockBand3', playRockBand3],
    ['readUserConfigFile', async () => await UserConfigAPI.readFile()],
    ...
    ['saveUserConfigFile', async (_, __, newConfig?: Partial<UserConfigObject>): Promise<string> => await UserConfigAPI.saveFile(newConfig)],
    ...
    ['testUserConfig', async (win): Promise<boolean> => await UserConfigAPI.validateFile(win)],
