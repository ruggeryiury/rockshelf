...
import { type DirPath, type DirPathLikeTypes, FilePath, type FilePathLikeTypes, execAsync, pathLikeToDirPath, pathLikeToFilePath, pathLikeToString } from 'node-lib'
...
import { EDATFile, type ImageConvertingOptions, ImageFile, type ImageFormatTypes, MOGGFile, RBTools } from '../core.exports'
import { type RB3CompatibleDTAFile, type TPLHeaderParserObject, genAudioFileStructure } from '../lib.exports'
...
export interface MIDIFileStatPythonObject {
	/**
	 * The encoding used for text events on the MIDI file.
	 */
	charset: string
	/**
	 * The MIDI type of the MIDI file. Always `1` for Rock Band MIDI files.
	 */
	midiType: number
	/**
	 * The ticks per beat of the MIDI file. Always `480` for Rock Band MIDI files.
	 */
	ticksPerBeat: number
	/**
	 * The amount of tracks inside the MIDI file (excluding the tempo track).
	 */
	tracksCount: number
	/**
	 * An array with the names of all tracks inside the MIDI file (excluding the tempo track).
	 */
	tracksName: string[]
}
...
export interface SongChartStatObject {
	/**
	 * The encoding used for text events on the MIDI file.
	 */
	charset: string
	...
	drums: {
		/**
		 * A flag that tells with the instrument is available on the song chart.
		 */
		enabled: boolean
		/**
		 * A flag that tells if the song has a solo of the specific instrument.
		 */
		hasSolo: boolean
		/**
		 * The drum mix event used for the song.
		 */
		mixEvent: string
		/**
		 * The amount of drum channels expected for the used drum mix event.
		 */
		channels: number
	}
	bass: {
		...
		hasSolo: boolean
		/**
		 * A flag that tells if the song has PRO parts authored for the specific insturment.
		 */
		hasPRO: boolean
	}
	guitar: {
	...
	vocals: {
		...
		hasSolo: boolean
		/**
		 * The amount of vocal parts available on the song chart.
		 */
		vocalParts: 0 | 1 | 2 | 3
	}
	keys: {
...
export class PythonAPI {
	...
	/**
	 * Returns an object with stats of a song chart. Both MIDI and decrypted EDAT files are supported.
	 * - - - -
	 * @param {FilePathLikeTypes} midiFilePath The path to the MIDI/Decrypted EDAT file
	 * @returns {Promise<SongChartStatObject>}
	 */
	static async songChartStat(midiFilePath: FilePathLikeTypes): Promise<SongChartStatObject> {
		const pythonScript = 'song_chart_stat.py'
		const command = `"${RockshelfFileSystemAPI.pythonEnvScriptFile().path}" "${pythonScript}" "${pathLikeToString(midiFilePath)}" -p`
		const cwd = is.dev ? RBTools.pyFolder.path : RBTools.pyFolder.path.replace(/(\.asar)([\\/])/, '.asar.unpacked$2')
		const { stderr, stdout } = await execAsync(command, { windowsHide: true, cwd })
		if (stderr) throw new Error(stderr)
		const returnValue = JSON.parse(stdout) as SongChartStatObject
		return returnValue
	}
