import { BinaryReader, FilePath, type FilePathJSONRepresentation, type FilePathLikeTypes, createHashFromBuffer, pathLikeToFilePath, randomBytesFromRanges } from 'node-lib'
import { temporaryFile } from 'tempy'
import { useDefaultOptions } from 'use-default-options'

import { BinaryAPI, MIDIFile, PythonAPI, type SongChartStatObject } from '../core.exports'
import { type EDATFileStatObject, type RockBandPS3TitleIDs, edatStat, ps3GameIDs } from '../lib.exports'
...
export class EDATFile {
	...
	static async genContentID(text: string, game: RockBandPS3TitleIDs = 'rb3'): Promise<string> {
		...
		if ((contentID + text).length > 36) {
			...
			for (let i = 0; i < diff; i++) {
				contentID += (await randomBytesFromRanges(1)).toString('hex')[0].toUpperCase()
			}
		} else contentID += text
	...
	/**
	 * Returns an object with stats of the song chart.
	 * - - - -
	 * @returns {Promise<EDATFileStatObject>}
	 */
	async getChartStat(): Promise<SongChartStatObject> {
		const isEncrypted = await this.isEncrypted()
		if (isEncrypted) {
			const tempMIDI = await this.decrypt({ devKLicHash: EDATFile.genDevKLicHash(this.path.gotoDir('../../').name), destPath: temporaryFile({ extension: 'mid' }) })
			const stats = await PythonAPI.songChartStat(tempMIDI.path)
			await tempMIDI.path.delete()
			return stats
		}
		return await PythonAPI.songChartStat(this.path)
	}
}
