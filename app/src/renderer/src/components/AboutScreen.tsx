    ...
    <AnimatedSection id="AboutScreen" condition={active} {...animate({ opacity: true })} className="absolute! z-10 h-full max-h-full w-full max-w-full bg-black p-8">
      <div className="h-full w-full overflow-y-auto">
        <div className="mb-4 flex-row! items-center">
          <img src="rbicons://website" className="mr-2 h-36 w-36 rounded-sm" />
          <div className="w-fill mr-auto">
            <h1 className="text-xl uppercase">{t('appTitle')}</h1>
            <h2 className="mb-2">{t('appDesc')}</h2>
            <h3 className="font-pentatonic mb-1 border-b border-white/25 pb-1 uppercase">{t('buildInfo')}</h3>
          ...
          <button
            ...
            className="absolute! top-0 right-0 mb-1 w-fit self-start rounded-xs border border-neutral-700 bg-neutral-900 px-1 py-0.5 text-xs! text-nowrap uppercase duration-100 hover:bg-neutral-700 active:bg-neutral-600 disabled:text-neutral-700 disabled:hover:bg-neutral-900"
            onClick={async () => {
