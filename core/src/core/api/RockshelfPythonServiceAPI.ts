...
import { spawnAsync } from 'node-lib'
...
export class RockshelfPythonServiceAPI {
	...
	static async checkInstalledPythonVersion(): Promise<void> {
		...
		const res = await spawnAsync(sysPyName, [`--version`])
		if (res.stderr) {
			const title = RockshelfLocaleAPI.t('pythonNotInstalledErrorTitle')
			const message = RockshelfLocaleAPI.t('pythonNotInstalledError')
			dialog.showErrorBox(title, message)
	...
	static async init(): Promise<void> {
		...
		if (!pythonMainDir.exists || !pythonExeFile.exists) {
			await spawnAsync(pythonExeFile.fullname, ['-m', 'venv', 'Python'], { cwd: userDataDir.path })
			await spawnAsync(pythonExeFile.fullname, ['-m', 'pip', 'install', '-r', RockshelfFileSystemAPI.coreModuleRootDir().gotoFile('bin/python/requirements.txt').path], { cwd: pythonScriptPath.path })
		}
	}
}
