import { readUserConfigFile, RockshelfFileSys, sendDialog, sendMessageBox, sendRendererConsole, useHandler } from '../core.exports'
...
export const downloadAndInstallDeluxe = useHandler(async (win, __, options: DeluxeInstallationOptions): Promise<false | DeluxeInstalledData> => {
  ...
  for (const entry of zip.files) {
    const dest = RockshelfFileSys.rb3UsrDir(devhdd0).gotoFile(`../${entry.path}`)
