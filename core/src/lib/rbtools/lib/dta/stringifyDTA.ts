...
import { type DTAFileKeys, type DTAFileUpdateObject, DTAIO, type DTAIOFormattingOptions, type FloatValueObject, type RB3CompatibleDTAFile, type SongSortingTypes, genTracksCountArray, sortDTA, tabNewLineFormatter } from '../../lib.exports'
	...
	if (updates.length > 0) {
		...
		for (const upd of updates) {
			...
			if (hasSongSpecific) {
				...
				if (tracks?.crowd !== undefined) songMap.set('crowd_channels', DTAIO.useArray(tracks.crowd, { ...io.options, array: { ...io.options.array, parenthesisForValues: false } }))
